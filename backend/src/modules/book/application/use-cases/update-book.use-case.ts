...
export class UpdateBookUseCase {
  ...
  async execute(id: number, input: UpdateBookDto): Promise<Result<BookResponseDto>> {
    try {
      ...
      existing.update({
        ...input,
        listPrice: input.listPrice ? new Prisma.Decimal(input.listPrice) : undefined,
        weight: input.weight ? new Prisma.Decimal(input.weight) : undefined,
      });

      const updated = await this.bookRepo.update(id, existing.toJSON() as any);
