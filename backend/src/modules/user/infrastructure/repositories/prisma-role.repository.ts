...
export class PrismaRoleRepository implements IRoleRepository {
    ...
    async create(data: { name: string; permissionIds: string[] }): Promise<RoleEntity> {
    ...
    private mapToEntity(role: any): RoleEntity {
        return {
            id: role.id,
            name: role.name,
            permissions: role.permissions.map((p: any) => this.mapPermissionToEntity(p)),
            createdAt: role.createdAt,
            updatedAt: role.updatedAt,
        };
    }

    private mapPermissionToEntity(permission: any): PermissionEntity {
        return {
            id: permission.id,
