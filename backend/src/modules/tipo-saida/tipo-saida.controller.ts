...
export class TipoSaidaController {
  ...
  async create(@Body() dto: CreateTipoSaidaDto) {
    ...
    if (existing) return { success: false, error: { code: 'TIPO_SAIDA_EXISTS', message: 'Tipo de saída já existe' } };
    ...
    if (dto.isVenda === true) {
      ...
      if (countVenda > 0) {
        return {
          success: false,
          error: { code: 'TIPO_SAIDA_IS_VENDA_EXISTS', message: 'Já existe um tipo de saída com isVenda = true' },
        };
      }
    }
  ...
  @Patch(':id')
  async update(@Param('id', ParseIntPipe) id: number, @Body() dto: UpdateTipoSaidaDto) {
    const item = await this.repo.findById(id);
    if (!item) throw new NotFoundException('Tipo de saída não encontrado');
