...
function renderCell(columnId: string, row: Language = language) {
  const columns = getLanguageTableColumns({
    onEdit: vi.fn(),
    onDelete: vi.fn(),
  });
...
describe('getLanguageTableColumns', () => {
  it('renders active and inactive status badges', () => {
    renderCell('isActive', { ...language, isActive: true });
    expect(screen.getByText('Ativo')).toBeInTheDocument();

    renderCell('isActive', { ...language, isActive: false });
  ...
  it('calls edit and delete handlers from action items', async () => {
    ...
    const columns = getLanguageTableColumns({ onEdit, onDelete });
    ...
    render(actions.cell({ row: { original: language } } as never) as ReactNode);
