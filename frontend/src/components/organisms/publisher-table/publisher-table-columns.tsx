...
interface GetPublisherTableColumnsParams {
  onEdit: (publisher: Publisher) => void;
  onDelete: (publisher: Publisher) => void;
}

export function getPublisherTableColumns({
  onEdit,
  onDelete,
}: GetPublisherTableColumnsParams): ColumnDef<Publisher>[] {
  return [
    {
      accessorKey: 'description',
      header: () => (
        <span className="text-foreground/80 font-semibold">Editora</span>
      ),
      cell: ({ row }) => (
